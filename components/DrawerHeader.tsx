      ...
      <View className="px-6 py-4">
        <View className="flex-row justify-between items-start mb-3">
          <View className="flex-1">
            ...
            <Text className="text-lg font-medium text-text-secondary mb-2">{formatCurrency(openingBalance, currency)}</Text>
            
            <View className="flex-row gap-4">
              <View>
                <Text className="text-xs text-text-muted uppercase tracking-wide mb-1">Net In</Text>
                <Text className="text-sm font-medium text-green-500">{formatCurrency(netIn, currency)}</Text>
              </View>
              <View>
                <Text className="text-xs text-text-muted uppercase tracking-wide mb-1">Net Out</Text>
                <Text className="text-sm font-medium text-red-500">{formatCurrency(netOut, currency)}</Text>
              </View>
            </View>
          </View>
          ...
          <View className="items-end">
            ...
            <Text className="text-2xl font-bold text-text">{formatCurrency(currentBalance, currency)}</Text>
          </View>
        </View>
        ...
        {closingBalance !== null && (
          <View className="border-t border-border pt-3">
            <View className="flex-row justify-between items-center mb-1">
              ...
              <Text className="text-xl font-semibold text-text">{formatCurrency(closingBalance, currency)}</Text>
            </View>
            {difference !== null && (
